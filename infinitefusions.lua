...
SMODS.Joker {
	...
	has_behaviour = function(card)
		if not card.infinifusion_api then return nil 
			...
			if api.redeem and type(api.redeem) == 'function' then  return true end
		end
		return nil
	end,
	...
	can_use = function(self, card)
		...
		if not self.has_behaviour(card) then
			...
			local api = card.infinifusion_api
			if (api.can_use and type(api.can_use) == 'function') and ((api.use and type(api.use) == 'function') or (api.redeem and type(api.redeem) == 'function')) then
				can_use_fus = api.can_use(self, card)
			elseif (api.use and type(api.use) == 'function') or (api.redeem and type(api.redeem) == 'function') then 
				can_use_fus = true
			end
		end
		return can_use_fus
	...
	use = function(self, card, area, copier)
		...
		if not self.has_behaviour(card) then
			...
			local api = card.infinifusion_api
			if api.redeem and type(api.redeem) == 'function' then 
				infinifusion_pseudoredeem(self, card, api.redeem)
			end
	...
	keep_on_use = function(self, card)
		...
		if not self.has_behaviour(card) then
			...
			calculate_infinifusion(card, nil, function(i)
				if (card.infinifusion[i].ability.consumeable or card.infinifusion[i].ability.set == 'Voucher') and keep_on_use then
...
function infinifusion_pseudoredeem(center, card, override)
	G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
		top_dynatext = DynaText({string = localize{type = 'name_text', set = center.set, key = center.key}, colours = {G.C.WHITE}, rotate = 1,shadow = true, bump = true,float=true, scale = 0.9, pop_in = 0.6/G.SPEEDFACTOR, pop_in_rate = 1.5*G.SPEEDFACTOR})
	...
	local center_back = card.config.center
	card.config.center = center
	
	if not override then
		card:apply_to_run()
	else
		override(card.config.center)
	end
	card.config.center = center_back
	delay(0.6)
